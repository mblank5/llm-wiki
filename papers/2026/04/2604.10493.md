...
authors: ["Mahir Labib Dihan", "Bangladesh University of Engineering and Technology (BUET)", "Bangladesh", "and", "Md Ashrafur Rahman Khan"]
...
Codebase: [https://github.com/mahirlabibdihan/swe-shepherd](https://github.com/mahirlabibdihan/swe-shepherd)
Dataset: [https://huggingface.co/datasets/mahirlabibdihan/swe-prm-collection](https://huggingface.co/datasets/mahirlabibdihan/swe-prm-collection)
Model: [https://huggingface.co/mahirlabibdihan/swe-shepherd-1.5b](https://huggingface.co/mahirlabibdihan/swe-shepherd-1.5b)

Figure: Figure 1. Overview of the SWE-Shepherd framework. The framework consists of six stages. (1) Training tasks are collected from SWE-Bench. (2) An LLM-based agent tries to solve the tasks by interacting with the codebase and generate solution trajectories composed of intermediate reasoning steps and actions. (3) Each intermediate step is assigned a scalar reward reflecting its contribution toward solving the task. (4) A dataset is constructed where the input text (problem description, execution history, and current action) is paired with the computed reward labels. (5) A Process Reward Model (PRM) is trained by feeding text representations from the LLM into an MLP head and optimizing with mean squared error (MSE) loss to predict step-level rewards. (6) During inference, the trained PRM guides reward-aware search, prioritizing higher-quality intermediate steps to improve problem-solving performance.
Refer to caption: 2604.10493v1/x1.png

## 2. Related Work

Recent work has explored the use of LLMs for autonomous software engineering, with benchmarks such as SWE-Bench(Jimenez et al., [n. d.]) and SWE-Bench Verified(OpenAI, 2024) highlighting the difficulty of long-horizon reasoning, repository-scale context understanding, and test-driven correctness.

Several approaches attempt to improve agent behavior using *reinforcement learning (RL)*. Methods such as SWE-RL(Wei et al., [n. d.]) and Agent-RLVR(Da et al., 2025) apply policy optimization or environment-driven reward signals to address sparse supervision and improve trajectory planning. In contrast, other work focuses on *data synthesis and supervised learning*, for example SWE-Synth(Pham et al., 2025), which generates structured bug-fix trajectories to provide dense training signals without RL.

Process-level reward modeling has recently emerged as an alternative paradigm for guiding multi-step agents. Web-Shepherd(Chae et al., 2025), for instance, predicts the utility of intermediate steps to steer decision-making in web navigation tasks. Our work extends this idea to repository-level software engineering by constructing PRMs tailored to code-editing environments and studying their effectiveness as a lightweight alternative to RL-based training.

## 3. Methodology

This section presents the SWE-Shepherd pipeline, covering task collection, trajectory generation, reward computation, dataset construction, process reward model training, and reward-guided inference (Figure [1](#S1.F1)).
...
Trajectories and rewards are converted into supervised training samples: problem statement, recent interaction history, candidate action, and scalar reward (normalized to $[0,1]$). The dataset contains over 15,000 samples covering diverse tasks.
...
- •
mini-SWE-Agent (min, 2026): A strong LLM agent that follows a sequential decision process without explicit search or reward modeling.
- •
SWE-Search (Antoniades et al., [n. d.]): A search-based framework that augments software agents with Monte Carlo Tree Search (MCTS) to explore multiple candidate action trajectories before committing to a solution.
- •
...
| Task Outcome | Average Reward |
| --- | --- |
| Resolved Tasks | 0.4894 |
| Unresolved Tasks | 0.4818 |
...
Accessed: 2026‑02‑20.
- Antoniades et al. ([n. d.])
Antonis Antoniades, Albert Örwall, Kexun Zhang, Yuxi Xie, Anirudh Goyal, and William Yang Wang. [n. d.].
SWE-Search: Enhancing Software Agents with Monte Carlo Tree Search and Iterative Refinement. In *The Thirteenth International Conference on Learning Representations*.
- Chae et al. (2025)
...
Web-Shepherd: Advancing PRMs for Reinforcing Web Agents. In *The Thirty-ninth Annual Conference on Neural Information Processing Systems*.
[https://openreview.net/forum?id=G2kMroO9UV](https://openreview.net/forum?id=G2kMroO9UV)
- Da et al. (2025)
Jeff Da, Clinton Wang, Xiang Deng, Yuntao Ma, Nikhil Barhate, and Sean Hendryx. 2025.
Agent-rlvr: Training software engineering agents via guidance and environment rewards.
*arXiv preprint arXiv:2506.11425* (2025).
- Jimenez et al. ([n. d.])
...
SWE-RL: Advancing LLM Reasoning via Reinforcement Learning on Open Software Evolution. In *The Thirty-ninth Annual Conference on Neural Information Processing Systems*.