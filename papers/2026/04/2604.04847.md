...
authors: ["Guan-Ting Lin", ",", "Chen Chen", "Zhehuai Chen", "Hung-yi Lee", "Correspondence:"]
...
Abstract We introduce Full-Duplex-Bench-v3 (FDB-v3) , a benchmark for evaluating spoken
...
We evaluate six model configurations: GPT-Realtime(^3^33https://developers.openai.com/api/docs/models/gpt-realtime-1.5), Gemini Live 2.5(^4^44https://ai.google.dev/gemini-api/docs/models/gemini-2.5-flash-native-audio-preview-12-2025), Gemini Live 3.1(^5^55https://ai.google.dev/gemini-api/docs/models/gemini-3.1-flash-live-preview), Grok(^6^66https://x.ai/news/grok-voice-agent-api), Ultravox v0.7(^7^77https://github.com/fixie-ai/ultravox), and a Cascaded baseline. All six are deployed through the LiveKit Realtime Voice Agent framework for streaming audio and real-time tool use. The first five are end-to-end speech-to-speech models; the Cascaded system follows a traditional modular pipeline—OpenAI Whisper for speech recognition, GPT-4o for reasoning and tool use, and OpenAI TTS for speech synthesis—serving as a reference point for the conventional architecture. Each model receives the same audio input.
...
- •
Argument Accuracy: Semantic correctness of generated arguments, judged by GPT-4o to accommodate valid input variations such as date formats, abbreviations, and dynamic variables from prior turns (Appendix [A.1](#A1.SS1)).
- •
...
- •
Response Quality: GPT-4o judges whether the agent’s spoken transcript accurately fulfills the user’s intent in natural language, penalizing correct tool execution that fails to relay results effectively (Appendix [A.2](#A1.SS2)).
- •
Turn-Taking and Latency Dynamics: From time-aligned execution logs, the Turn-take rate measures the fraction of turns with a natural-timing response. Base latency is $\Delta t=t_{\text{agent_start}}-t_{\text{user_end}}$; if $\Delta t<0$, the event is an Interruption.
...
3.
Task Completion Latency: Time until the agent delivers the factual answer. GPT-4o analyzes ASR chunks to isolate the informational sentence from preceding filler speech (Appendix [A.3](#A1.SS3)).
...
Table [2](#S4.T2) summarises aggregate scores. GPT-Realtime is the strongest overall performer, leading on tool selection F1 (0.876), argument accuracy (0.680), response quality (0.792), Pass@1 (0.600), and the lowest interruption rate (13.5%).
...
| Gemini Live 2.5 | 0.483 | 0.760 | 0.231 | 0.500 |
| Gemini Live 3.1 | 0.448 | 0.920 | 0.269 | 0.550 |
| Grok | 0.414 | 0.760 | 0.115 | 0.450 |
| Ultravox | 0.345 | 0.680 | 0.192 | 0.450 |
| Cascaded | 0.414 | 0.800 | 0.192 | 0.400 |

### 5.3 Robustness to Disfluency

Table [3](#S5.T3) conditions Pass@1 on disfluency type. GPT-Realtime leads or ties on every category, with a particularly strong self-correction score (0.588) that substantially exceeds all other systems. Gemini Live 2.5 is second on self-corrections (0.471) but lags on false starts (0.417); Gemini Live 3.1 is more balanced but trails on self-corrections (0.353), suggesting the version update improved general robustness without improving state rollback. Pauses are the weakest category across most systems (Grok and Ultravox both at 0.333), highlighting a shared difficulty in detecting whether a user has finished speaking.

### 5.4 Performance by Domain

Table [5](#S5.T5) breaks down Pass@1 by domain. Finance is the easiest domain for all models (GPT-Realtime 0.960, Gemini Live 3.1 0.920); Housing is the hardest (GPT-Realtime leads at 0.308, Grok at 0.115). GPT-Realtime leads on all four domains. Ultravox shows notable domain imbalance: its E-commerce (0.345) and Housing (0.192) scores are among the lowest, pointing to weaknesses in multi-entity order handling and complex constraint reasoning.

### 5.5 Latency Analysis

Table [6](#S5.T6) decomposes latency into first word, tool call, and task completion. Gemini Live 3.1 is fastest on all three (3.95 s / 2.21 s / 4.25 s), though this speed likely contributes to its low turn-take rate. GPT-Realtime maintains moderate latency (6.89 s) alongside the best accuracy. Grok is competitive (6.65 s) with the fastest first-word response among non-Gemini systems.
...
Table [7](#S6.T7) shows finance_18, a three-tool chain: convert 1,000 USD to EUR, update credit-card autopay to savings, and check premium-card benefits. No disfluency is present—the user issues all three requests clearly in a single utterance.
...
| Model | Tool | Arg | Resp | 1^st Resp | Tool Call | Compl. |
| --- | --- | --- | --- | --- | --- | --- |
|  | Sel | Acc | Qual | (s) | (s) | (s) |
| GPT-RT | 1.00 | 1.00 | 1.00 | 8.48 | 6.40 | 9.20 |
| Gem 2.5 | 1.00 | 1.00 | 1.00 | 6.16 | 3.60 | 6.16 |
| Gem 3.1 | 1.00 | 1.00 | 1.00 | 3.36 | 2.43 | 3.92 |
| Grok | 1.00 | 1.00 | 1.00 | 5.20 | 2.39 | 5.92 |
| Ultravox | 1.00 | 0.67 | 0.00 | 2.64 | 6.31 | 8.16 |
| Cascaded | 1.00 | 0.67 | 0.00 | $-$2.40 | — | 8.64 |
...
Table [8](#S6.T8) presents travel_19, where the user corrects both destination (Rome$\rightarrow$Milan) and date (June 1$\rightarrow$June 3) mid-utterance.
...
| --- | --- | --- | --- | --- | --- | --- |
|  | Sel | Acc | Qual | (s) | (s) | (s) |
| GPT-RT | 1.00 | 1.00 | 1.00 | 4.40 | 2.55 | 4.40 |
| Gem 2.5 | 1.00 | 1.00 | 0.00 | 5.28 | 4.23 | 5.28 |
| Gem 3.1 | 1.00 | 0.00 | 0.00 | 2.56 | $-$2.27 | 2.56 |
| Grok | 0.50 | 0.00 | 1.00 | — | $-$2.47 | — |
| Ultravox | 1.00 | 1.00 | 0.00 | $-$1.36 | $-$0.18 | 3.04 |
| Cascaded | 0.67 | 0.00 | 0.00 | 8.08 | 1.79 | 8.08 |
...
Technical report
Kyutai.
External Links: [Link](http://kyutai.org/Moshi.pdf)
Cited by: [§1](#S1.p1.1).
...
External Links: [Link](https://aclanthology.org/2025.findings-emnlp.241/),
[Document](https://dx.doi.org/10.18653/v1/2025.findings-emnlp.241),
...
Vol. , pp. 1060–1067.
External Links: [Document](https://dx.doi.org/10.1109/SLT61566.2024.10832184)
Cited by: [§1](#S1.p3.1),
[§2.2](#S2.SS2.p1.1).
...
Note: Preprint
External Links: [Link](https://github.com/DanielLin94144/Full-Duplex-Bench)
Cited by: [§1](#S1.p4.1),
[§2.1](#S2.SS1.p1.1).
...
External Links: [Link](https://openreview.net/forum?id=Q7lAqY41HH)
