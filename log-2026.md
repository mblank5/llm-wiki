...
## [2026-05-12] source-repair | DeepXiv raw fallback
- Confirmed `deepxiv` is installed as a local CLI and supports `search`, `trending`, `paper --head`, `paper --brief`, and `paper --raw`.
- Added `scripts/fetch-deepxiv-raw.py` as a repeatable fallback for weak `arxiv2md` captures.
- Repaired four weak paper sources with `deepxiv paper <id> --raw`:
  - `2601.04720` Qwen3-VL-Embedding / Reranker
  - `2604.15774` MemEvoBench
  - `2604.11563` Synthius-Mem
  - `2411.18138` SALMONN-omni
- Regenerated their Chinese paper cards; paper-card state is now 256 complete Chinese cards and 0 source-limited cards.
