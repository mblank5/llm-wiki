...
- Generated `paper-briefs` for all 256 paper records:
  - 149 full Chinese LLM-generated paper cards.
  - 3 source-limited cards for genuinely tiny/incomplete source files.
  - 104 pending Chinese cards because the configured LongCat AppId hit its usage limit during batch generation.
- Added `--fallback-missing` so the site can avoid raw English dumps when model quota is temporarily unavailable; rerun the script later to replace pending cards with full briefs.
