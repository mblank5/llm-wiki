...
ArXiv: 2603.25562
Authors: Yuqian Fu Haohuan Huang, Kaiwen Jiang Yuanheng Zhu, Dongbin Zhao, State Key Laboratory of Multimodal Artificial Intelligence Systems, CASIA, School of Artificial Intelligence, UCAS, Corresponding authors., Work in progress.
Sections: 44
Estimated tokens: 12.1k
...
We refer to ([2](#S3.E2)) as token-level OPD.
...
Under bounded rewards and bounded score-function gradients, the worst-case variance upper bound of token-level OPD scales as $O(T^{2})$, whereas the sequence-level estimator scales as $O(T^{4})$. We provide a detailed derivation in Appendix [B](#A2).
...
We conduct a two-task toy experiment, where increasing $\gamma$ is observed to induce substantially higher gradient variance and less stable optimization; see Figure [1](#S3.F1) for an illustration and Appendix [C](#A3) for additional experimental details.

Figure: (a) Gradient variance in the toy experiment. Larger $\gamma$ generally yields higher variance in both tasks.
Refer to caption: 2603.25562v1/x2.png
...
As shown in Figure [2](#S3.F2), most sampled tokens receive negative rewards, and the positive learning signal is concentrated on a relatively small subset of tokens with positive advantage.
...
Figure: Figure 3: The student falls into a repetition loop, but the teacher model maintains highly aligned with the student model on the repeating tokens, indicating a lack of proper penalty for such behavior.
Refer to caption: 2603.25562v1/x5.png
...
The same failure also appears in how the teacher signal changes with position. Figure [4](#S3.F4) shows the distribution of teacher-student log-probability gaps across token positions; it is relatively concentrated at early positions and becomes progressively wider later in the sequence, with more extreme values on long rollouts.

Figure: Figure 4: Distribution of teacher-student log-probability gaps across token positions. Later positions show wider distributions and more extreme values, indicating a noisier teacher signal on long student-generated rollouts.
Refer to caption: 2603.25562v1/x6.png
...
Figure: Figure 5: Token-level comparison can penalize semantically correct outputs due to tokenizer mismatch.
Refer to caption: 2603.25562v1/x7.png
...
Section [4.1](#S4.SS1) introduces the objective, and Section [4.2](#S4.SS2) describes the practical choices that ensure stable training.
...
Table [2](#S5.T2) shows a more asymmetric pattern in alternating multi-task training. The sampled-token OPD baseline is already strong on ALFWorld, so the main room for improvement lies on the math side.
...
Figure: Figure 6: Single-task training curves for math reasoning. Local support matching improves training reward and final evaluation over the course of training.
Refer to caption: 2603.25562v1/x8.png

Figure: Figure 7: Multi-task learning curves for ALFWorld and math reasoning. The main gains appear on the math side while agentic performance remains strong.
Refer to caption: 2603.25562v1/x9.png

Figure: (a) Single-task optimization statistics.
Refer to caption: 2603.25562v1/x10.png

Figures [6](#S5.F6), [7](#S5.F7), and [8](#S5.F8) provide a more detailed view of the optimization dynamics.
...
Cited by: [§2](#S2.p1.1).
- C. Ye, X. Zhang, Y. Hao, Z. Yu, Z. Zhang, A. Gullapalli, H. Chen, and T. Zhang (2026)
...
Figure [A4](#A4.F4) illustrates several representative cases.

Figure: (a) High teacher probability on generic reasoning fillers (implies) at step 5.
Refer to caption: 2603.25562v1/x23.png

The trajectory then develops into *hesitation loops and low-information continuations*. Repeated wait tokens, punctuation-heavy continuations, and other semantically weak fillers can remain locally rewardable even after the overall trajectory has become unproductive. This is consistent with the repetition-loop discussion in Section [3.2](#S3.SS2). We provide two similar cases in Figure [A5](#A4.F5).

Figure: (a) The teacher may fail to penalize, and sometimes even reinforce, repetitive generation.
Refer to caption: 2603.25562v1/x25.png

Finally, once the student drifts further off-distribution, the local signal can remain misleadingly positive rather than self-correcting. In the case study, this appears as degeneration and gibberish outputs, yet many tokens still receive high teacher probability. An example is shown in Figure [A6](#A4.F6).

Figure: Figure A6: The teacher still assigns high probability to several tokens after the student drifts into nonsensical Chinese outputs.
Refer to caption: 2603.25562v1/x27.png