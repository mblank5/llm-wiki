...
arXiv:2604.17535v1 [cs.CL] 19 Apr 2026

# OPSDL: On-Policy Self-Distillation for Long-Context Language Models

Xinsen Zhang1, Zhenkai Ding1, Tianjun Pan1, Run Yang1, Chun Kang1,

Xue Xiong1, Jingnan Gu1

1Baidu Inc

###### Abstract

Extending the effective context length of large language models (LLMs) remains a central challenge for real-world applications.
...
We propose OPSDL, an On-Policy Self-Distillation method for enhancing the Long-context capabilities of LLMs.
...
Let CLC_{L} denote the long-context input, and CSC_{S} denote the corresponding short context extracted from CLC_{L}, which contains the core information. Based on CSC_{S}, an instruction or query QQ is formulated. Given the query QQ and the context, the large language model, parameterized by θ\theta and denoted as πθ\pi_{\theta}, auto-regressively generates a response sequence y={y1,y2,…,yT}y=\{y_{1},y_{2},\dots,y_{T}\}.

The key insight of our approach is that the same policy πθ\pi_{\theta}, when prompted with the short-context input CSC_{S}, produces a more calibrated and accurate next-token distribution, since CSC_{S} avoids distracting noise and fits perfectly within the model’s well-trained context window. This allows the model to leverage its well-aligned short-context capabilities to self-align in long-context scenarios.

Therefore, instead of relying on external reward models, we formulate the long-context optimization problem as a self-alignment process: we aim to optimize θ\theta such that the model’s generation distribution under the long context, πθ​(y∣CL,Q)\pi_{\theta}(y\mid C_{L},Q), closely approximates its own high-quality distribution under the short context, πθ​(y∣CS,Q)\pi_{\theta}(y\mid C_{S},Q).

###
3.2 Data Construction

We constructs (CL,CS,Q)(C_{L},C_{S},Q) triplets from raw long documents without human annotation.

We sample a long document CLC_{L} from the training corpus, then extract a contiguous segment CS⊂CLC_{S}\subset C_{L} with |Cs|≤|CL||C_{s}|\leq|C_{L}| that preserves the core evidence while fitting within the model’s well-trained context window.
Finally, we generate a question Q∼πθ(⋅∣CS)Q\sim\pi_{\theta}(\cdot\mid C_{S}) conditioned on CSC_{S}, ensuring that QQ targets evidence present in both contexts.
This yields triplets where the question QQ is answerable from both CLC_{L} and CSC_{S}, enabling meaningful comparison between the short-context teacher and the long-context student.
...
the short context CSC_{S} as a robust anchor. Formally, for a response
y=(y1,…,yT)y=(y_{1},\ldots,y_{T}) sampled from the long-context student policy
πθ(⋅∣CL,Q)\pi_{\theta}(\cdot\mid C_{L},Q), the teacher probability at each token position is:

|

P​(yt)=πTeacher​(yt∣CS,Q,y<t),P(y_{t})=\pi_{\text{Teacher}}(y_{t}\mid C_{S},Q,y_{<t}),

(1)

where the same model parameters θ\theta are used under the short context,
...
a token-level advantage At​(yt)A_{t}(y_{t}) as the log-probability ratio between the
short-context (teacher) and long-context (student) distributions:

|

At​(yt)=log⁡πTeacher​(yt∣CS,Q,y<t)πθ​(yt∣CL,Q,y<t),A_{t}(y_{t})=\log\frac{\pi_{\text{Teacher}}(y_{t}\mid C_{S},Q,y_{<t})}{\pi_{\theta}(y_{t}\mid C_{L},Q,y_{<t})},
...
Positive At​(yt)>0A_{t}(y_{t})>0: the teacher assigns higher
...
Negative At​(yt)<0A_{t}(y_{t})<0: the student assigns higher
...
Near-zero At​(yt)≈0A_{t}(y_{t})\approx 0: both distributions agree,
...
distribution πθ(⋅∣CL,Q)\pi_{\theta}(\cdot\mid C_{L},Q):

|

ℒPG​(θ)=−𝔼y∼πθ(⋅∣CL,Q)​[∑t=1TAt​(yt)⋅log⁡πθ​(yt∣CL,Q,y<t)],\mathcal{L}_{\mathrm{PG}}(\theta)=-\mathbb{E}_{y\sim\pi_{\theta}(\cdot\mid C_{L},Q)}\left[\sum_{t=1}^{T}A_{t}(y_{t})\cdot\log\pi_{\theta}(y_{t}\mid C_{L},Q,y_{<t})\right],

(3)

where the advantage At​(yt)A_{t}(y_{t}) is defined in Eq. (2).
...
training triples ⟨CL,CS,Q⟩\langle C_{L},C_{S},Q\rangle, and a training phase
that iteratively optimizes the policy via token-level advantage estimation.

Algorithm 1  OPSDL Algorithm

0: Long-document corpus 𝒟\mathcal{D}; Initial LLM πθ\pi_{\theta} to be optimized; Short-context-aligned LLM πTeacher\pi_{\text{Teacher}}.

0: Optimized LLM πθ\pi_{\theta} with longer effective context length

1: // Data Construction Phase

2: Sample a long context CL∼𝒟C_{L}\sim\mathcal{D}

3: Extract short chunk CS⊂CLC_{S}\subset C_{L} and generate query QQ via Self-QA based on CSC_{S}

4: Construct the training triple ⟨CL,CS,Q⟩\langle C_{L},C_{S},Q\rangle
...
8:  Generate response y={y1,y2,…,yT}∼πθ(⋅∣CL,Q)y=\{y_{1},y_{2},\dots,y_{T}\}\sim\pi_{\theta}(\cdot\mid C_{L},Q)

9:  // Token-level Reverse-KL Advantage Estimation

10:  for each token yty_{t} do

11:   Compute student log-prob: st=log⁡πθ​(yt∣CL,Q,y<t)s_{t}=\log\pi_{\theta}(y_{t}\mid C_{L},Q,y_{<t})

12:   Compute teacher log-prob: tt=log⁡πTeacher​(yt∣CS,Q,y<t)t_{t}=\log\pi_{\text{Teacher}}(y_{t}\mid C_{S},Q,y_{<t})

13:   At=tt−stA_{t}=t_{t}-s_{t} = log⁡PteacherPstudent\log\frac{P_{\text{teacher}}}{P_{\text{student}}}

14:  end for

15:  // Policy Optimization

16:  Update πθ\pi_{\theta} using token-level advantages {At}\{A_{t}\}

17: end for

18: return  πθ\pi_{\theta}
...
Following the data preparation pipeline in LongPO( Chen et al. (2025)), we construct training data from raw long documents without human annotation. We adopt a reverse construction strategy: for each long document CLC_{L}, we first randomly sample a shortened chunk CSC_{S} and then prompt the model to generate instructions based on the CSC_{S} via Self-Instruct( Wang et al. (2023)). To ensure diversity, the model first generates an instruction pool and then randomly samples an instruction II from it. In contrast to LongPO, our method only requires the (CL,CS,I)(C_{L},C_{S},I) triplets and does not require any preference responses. Specifically, our method trains on responses sampled on-policy from the model itself, which not only simplifies data preparation but also mitigates the distribution mismatch issues associated with off-policy data.
