...
arXiv:2604.14084v2 [cs.LG] 19 Apr 2026

# TIP: Token Importance in On-Policy Distillation

Yuanda Xu
Hejian Sang11footnotemark: 1  Zhengze Zhou11footnotemark: 1  Ran He11footnotemark: 1  Zhipeng Wang  Alborz Geramifard
...
We ask: which tokens carry the most useful learning signal in OPD?
Our answer is that informative tokens come from two regions: positions with high student entropy, and positions with low student entropy plus high teacher–student divergence—where the student is overconfident and wrong.
...
We organize these findings with TIP (Token Importance in on-Policy distillation), a two-axis taxonomy over student entropy and teacher–student divergence that explains why entropy is useful yet structurally incomplete and motivates type-aware selection rules that combine uncertainty and disagreement.
...
We organize this picture with TIP, a two-axis taxonomy crossing student entropy and teacher–student divergence into four quadrants (Section 4).
Theoretically, entropy is a strong first-order proxy but must conflate “confident and correct” with “confident and wrong,” and a parameter-free Soft-OR score fixes this blind spot (Section 5).
Experimentally, the combined score consistently improves over entropy-only selection on mathematical reasoning and remains competitive on agentic planning, where Q3-only selection is strongest (Section 7).
...
We prove that entropy is a strong first-order proxy but any entropy-only score is structurally blind to overconfident tokens, and that a parameter-free Soft-OR score fixes this blind spot (Propositions 1–2, Remark 2).
...
Figure 1: Cross-task summary: average accuracy by selection method.
Each panel shows one benchmark; bar height is the mean accuracy (mean@16) averaged across three teacher–student pairs for mathematical reasoning (Qwen3-8B→\to4B, Llama-70B→\to8B, Qwen2.5-14B→\to1.5B) and across two teacher sizes (14B, 32B) for DeepPlanning.
Methods: Base. = all-token OPD (100%); Ent. 50%/20% = entropy-based token selection at the stated retention ratio; SO 50%/20% = Soft-OR (Eq. 5) top-kk selection.
...
Most closely related is AdaKD (Xie et al., 2026), whose LATF module performs hard top-rr% selection by teacher–student Hellinger distance—a divergence-only view we ablate in Appendix B.2.
Our Q3 specification is not a relabeling of “large-divergence tokens”: it is defined by the conjunction of low student entropy and high disagreement, and the two axes induce different selections (Table 3 vs. Table 8).
The AdaKD ablation itself supports this: LATF alone yields only +0.04+0.04 average ROUGE-L on Qwen2-1.5B (their Table 3a), with AdaKD’s full gain coming from an orthogonal temperature-scaling module (IDTS).
Our budget-matched comparison (Appendix B.2) reaches the same conclusion—divergence-only selection underperforms the baseline at equal budget, whereas the low-entropy + high-divergence selector remains competitive.
...
Let TT denote a frozen teacher and SθS_{\theta} a trainable student over vocabulary VV.
A prompt x∼𝒟x\sim\mathcal{D} is drawn, the student generates a rollout 𝐲=(y1,…,ym)∼Sθ(⋅∣x)\mathbf{y}=(y_{1},\ldots,y_{m})\sim S_{\theta}(\cdot\mid x), and the teacher scores each position.
The context at position tt is ct=(x,y<t)c_{t}=(x,y_{<t}).
The standard on-policy distillation loss is:

|

ℒ=1m∑t=1mDKL(PS(⋅∣ct)∥PT(⋅∣ct)).\mathcal{L}=\frac{1}{m}\sum_{t=1}^{m}D_{\mathrm{KL}}\!\left(P_{S}(\cdot\mid c_{t})\,\|\,P_{T}(\cdot\mid c_{t})\right).
...
ht=H(PS(⋅∣ct))log⁡|V|∈[0,1].h_{t}=\frac{H\bigl(P_{S}(\cdot\mid c_{t})\bigr)}{\log|V|}\in[0,1].

(2)

High hth_{t} means the student is uncertain; low hth_{t} means it is confident.

#### Teacher–student divergence.

|

δt=DKL(PS(⋅∣ct)∥PT(⋅∣ct)).\delta_{t}=D_{\mathrm{KL}}\!\left(P_{S}(\cdot\mid c_{t})\,\|\,P_{T}(\cdot\mid c_{t})\right).

(3)

High δt\delta_{t} means the teacher disagrees with the student.
...
The empirical question of this paper is whether useful training signal concentrates in particular regions of the (ht,δt)(h_{t},\delta_{t}) plane.

##
4 TIP Taxonomy: A Two-Axis View of Token Importance

We organize token importance along two axes already computed during standard OPD training: student entropy hth_{t} and teacher–student divergence δt\delta_{t}.
Crossing them yields four quadrants (Table 1, Figure 2).
...
Table 1: Token taxonomy. Classification by student entropy hth_{t} and teacher–student divergence δt\delta_{t}.
...
|
Q1: High entropy, high divergence
High
High
Correct errors or consolidate fragile knowledge

|
Q2: High entropy, low divergence
High
Low
Stabilize underconfident predictions

|
Q3: Overconfident
Low
High
Break systematic confident biases
...
We formalize these below and test each one experimentally in Section 7.
Specifically, we prove: (1) an oracle token weight favors Q1 >> Q2 >> Q3 ≫\gg Q4 (Proposition 1); (2) entropy-only scores are structurally blind to Q3 (Proposition 2); and (3) augmenting entropy with divergence restores coverage of all informative quadrants (Remark 2).

###
5.1 Oracle Token Weight

We want to identify which tokens most accelerate training.
We formalize this as: what per-token weights {wt}\{w_{t}\} minimize the expected loss after one gradient step?

Let gt=∇θℓtg_{t}=\nabla_{\theta}\ell_{t} be the per-token gradient, μ¯t=𝔼​[gt]\bar{\mu}_{t}=\mathbb{E}[g_{t}], and define
ϕ¯t=⟨∇L,μ¯t⟩\bar{\phi}_{t}=\langle\nabla L,\bar{\mu}_{t}\rangle and M¯t=𝔼​[‖gt‖2]\bar{M}_{t}=\mathbb{E}[\|g_{t}\|^{2}].
Under β\beta-smoothness and a token-separable approximation that neglects cross-token covariance terms (Appendix A.1), a weighted step g^=∑twt​gt\hat{g}=\sum_{t}w_{t}g_{t} satisfies the surrogate bound:

|

𝔼​[L​(θ−η​g^)]−L​(θ)≲∑t=1m(−η​wt​ϕ¯t+η2​β2​wt2​M¯t).\mathbb{E}[L(\theta-\eta\hat{g})]-L(\theta)\;\lesssim\;\sum_{t=1}^{m}\Bigl(-\eta\,w_{t}\,\bar{\phi}_{t}+\frac{\eta^{2}\beta}{2}\,w_{t}^{2}\,\bar{M}_{t}\Bigr).
...
The bound is minimized at wt∗=ϕ¯t/(η​β​M¯t)w_{t}^{*}=\bar{\phi}_{t}/(\eta\beta\bar{M}_{t}),
with per-token descent Δt∗=−ϕ¯t2/(2​β​M¯t)\Delta_{t}^{*}=-\bar{\phi}_{t}^{2}/(2\beta\bar{M}_{t}).

Indeed, the bound is separable across tokens, so each coordinate minimizes
−η​wt​ϕ¯t+η2​β2​wt2​M¯t-\eta w_{t}\bar{\phi}_{t}+\frac{\eta^{2}\beta}{2}w_{t}^{2}\bar{M}_{t} independently. Differentiating gives
−η​ϕ¯t+η2​β​wt​M¯t=0-\eta\bar{\phi}_{t}+\eta^{2}\beta w_{t}\bar{M}_{t}=0, hence
wt∗=ϕ¯t/(η​β​M¯t)w_{t}^{*}=\bar{\phi}_{t}/(\eta\beta\bar{M}_{t}). Substituting back yields
Δt∗=−ϕ¯t2/(2​β​M¯t)\Delta_{t}^{*}=-\bar{\phi}_{t}^{2}/(2\beta\bar{M}_{t}).
...
Q1: Large ϕ¯t\bar{\phi}_{t} (strong correction), moderate M¯t\bar{M}_{t} (well-conditioned) ⇒\Rightarrow largest wt∗w_{t}^{*}.

-
•

Q2: Moderate ϕ¯t\bar{\phi}_{t} (mild correction) ⇒\Rightarrow moderate wt∗w_{t}^{*}.

-
•

Q3: Positive ϕ¯t\bar{\phi}_{t} (real corrective signal despite low entropy) ⇒\Rightarrow positive wt∗w_{t}^{*}.

-
•

Q4: Near-zero ϕ¯t\bar{\phi}_{t} ⇒\Rightarrow negligible wt∗w_{t}^{*}.

The qualitative ordering is Q1>Q2>Q3≫Q4\text{Q1}>\text{Q2}>\text{Q3}\gg\text{Q4}.

######
Remark 1 (Quadrant ordering).

The ordering follows from the structure of each quadrant.
At high divergence, the teacher’s correction is misaligned with the student’s current prediction, so ϕ¯t\bar{\phi}_{t} is large and positive.
At high entropy, the student’s distribution is diffuse, spreading gradient energy across many vocabulary entries; M¯t\bar{M}_{t} is moderate, yielding a favorable ϕ¯t2/M¯t\bar{\phi}_{t}^{2}/\bar{M}_{t} ratio for Q1.
At low entropy (ht≈0h_{t}\approx 0), the distribution is sharply peaked, so M¯t\bar{M}_{t} is small—but Q3 still has positive ϕ¯t\bar{\phi}_{t} because the teacher strongly disagrees, giving wt∗>0w_{t}^{*}>0.
Q4 has both small ϕ¯t\bar{\phi}_{t} (teacher agrees) and small M¯t\bar{M}_{t}, so wt∗≈0w_{t}^{*}\approx 0.

In practice, wt∗w_{t}^{*} is unavailable because it depends on population-level quantities.
A natural proxy is student entropy hth_{t}, but any such score is structurally blind to Q3:

######
Proposition 2 (Blind spot).

Let w^​(ht)=f​(ht)\hat{w}(h_{t})=f(h_{t}) be any non-decreasing score with f​(0)=0f(0)=0 (e.g., f​(h)=hf(h)=h or f​(h)=𝟙​[h≥τ]f(h)=\mathbbm{1}[h\geq\tau]). Then Q3 tokens—which may have wt∗>0w_{t}^{*}>0—receive w^​(ht)≈0\hat{w}(h_{t})\approx 0. Entropy alone cannot distinguish “confident and correct” (Q4) from “confident and wrong” (Q3).

Appendix B.5 illustrates this concretely: Examples 1, 3, and 4 show Q3 tokens with ht<0.4h_{t}<0.4 that an entropy-only rule would discard, while Examples 2 and 5 show the contrasting high-entropy Q1 cases that entropy-based rules do capture.

Since divergence δt\delta_{t} is already computed as part of the loss, the natural fix is a score that is nonzero whenever either axis is active.
We define the Soft-OR score with min-max normalized inputs h^t,δ^t∈[0,1]\hat{h}_{t},\hat{\delta}_{t}\in[0,1]:

|

st=h^t+δ^t−h^t⋅δ^t=1−(1−h^t)​(1−δ^t).s_{t}=\hat{h}_{t}+\hat{\delta}_{t}-\hat{h}_{t}\cdot\hat{\delta}_{t}=1-(1-\hat{h}_{t})(1-\hat{\delta}_{t}).

(5)

This is parameter-free: sts_{t} is nonzero whenever either entropy or divergence is nonzero, without a tuning coefficient.

######
Remark 2 (Soft-OR fixes the blind spot).

For any Q3 token with h^t≈0\hat{h}_{t}\approx 0 and δ^t>0\hat{\delta}_{t}>0, the entropy proxy gives w^0​(ht)≈0\hat{w}_{0}(h_{t})\approx 0 (Proposition 2), but st≈δ^t>0s_{t}\approx\hat{\delta}_{t}>0.
Simultaneously, Q4 tokens (h^t≈0\hat{h}_{t}\approx 0, δ^t≈0\hat{\delta}_{t}\approx 0) remain suppressed: st≈0s_{t}\approx 0.
Q1 tokens retain the highest scores because both h^t\hat{h}_{t} and δ^t\hat{\delta}_{t} are large (st≈1s_{t}\approx 1).
The Soft-OR score therefore tracks the oracle ordering Q1 >> Q2 >> Q3 ≫\gg Q4 without requiring ϕ¯t\bar{\phi}_{t} or M¯t\bar{M}_{t}.
...
Q1/Q2 carry the most signal; removing Q4 improves efficiency
...
Entropy-only selection misses Q3 tokens
...
Combined score recovers Q3 and outperforms entropy-only
...
Given a retention ratio ρ∈(0,1]\rho\in(0,1], we retain the top-ρ\rho fraction of tokens by the Soft-OR score st=h^t+δ^t−h^t⋅δ^ts_{t}=\hat{h}_{t}+\hat{\delta}_{t}-\hat{h}_{t}\cdot\hat{\delta}_{t} (Equation 5):

|

𝒯=TopK​({st}t=1m,⌊ρ​m⌋).\mathcal{T}=\mathrm{TopK}\bigl(\{s_{t}\}_{t=1}^{m},\;\lfloor\rho m\rfloor\bigr).
...
ℒTIP=1|𝒯|∑t∈𝒯DKL(PS(⋅∣ct)∥PT(⋅∣ct)).\mathcal{L}_{\mathrm{TIP}}=\frac{1}{|\mathcal{T}|}\sum_{t\in\mathcal{T}}D_{\mathrm{KL}}\!\left(P_{S}(\cdot\mid c_{t})\,\|\,P_{T}(\cdot\mid c_{t})\right).

(7)

Setting δ^t=0\hat{\delta}_{t}=0 recovers entropy-only selection; including δ^t\hat{\delta}_{t} additionally promotes Q3 tokens.
The score is parameter-free and both hth_{t} and δt\delta_{t} are already computed during standard distillation, so the only extra cost is a min-max normalization and the top-kk sort—O​(m​log⁡m)O(m\log m) per rollout, negligible compared with forward and backward passes.

##
7 Experiments

We now validate each prediction of the taxonomy.
Table 2 maps each theoretical result to its experimental test; we proceed from the strongest signal (high-entropy tokens, Section 7.2) to the blind spot (Q3, Section 7.3) to the combined score (Section 7.4).
...
Qwen2.5: Qwen2.5-14B-Instruct-thinking →\to Qwen2.5-1.5B-Instruct (Qwen et al., 2025) (∼9×{\sim}9\times capacity gap, reasoning teacher)
...
All models are trained with AdamW, a cosine schedule, and reverse KL on student-generated rollouts (lr =1×10−6=1\times 10^{-6} for Qwen3 and Qwen2.5; lr =3×10−7=3\times 10^{-7} for Llama).
...
Table 3 (and Figure 3 in the Appendix) confirm this prediction.
...
This indicates that many low-entropy tokens are effectively solved (Q4) and mainly dilute the gradient; Appendix B.5 gives token-level intuition for the contrasting high-entropy cases that do carry corrective signal (Examples 2 and 5).
...
Table 3: Entropy sampling across model pairs. Accuracy (%, mean@16 ±\pm std). Sampling selects tokens with probability pt∝htp_{t}\propto h_{t}. Bold marks the best per benchmark.
...
Compute per-token forward KL: δtfwd=DKL(PT(⋅∣ct)∥PS(⋅∣ct))\delta_{t}^{\mathrm{fwd}}=D_{\mathrm{KL}}(P_{T}(\cdot\mid c_{t})\|P_{S}(\cdot\mid c_{t})).

-
2.

Compute per-token student entropy hth_{t} (Equation 2) and min-max normalize to [0,1][0,1]: h^t=(ht−hmin)/(hmax−hmin)\hat{h}_{t}=(h_{t}-h_{\min})/(h_{\max}-h_{\min}).

-
3.

Define confidence as conft=1−h^t\mathrm{conf}_{t}=1-\hat{h}_{t} (low entropy ⇒\Rightarrow high confidence).

-
4.

Compute the Q3 score: wtQ3=δtfwd⋅conftw_{t}^{\mathrm{Q3}}=\delta_{t}^{\mathrm{fwd}}\cdot\mathrm{conf}_{t}.

Tokens with high wtQ3w_{t}^{\mathrm{Q3}} are precisely the positions where the student is highly confident while the teacher strongly disagrees.
Conceptually, the taxonomy defines Q3 using the reverse-KL disagreement axis in Equation 3; operationally, this experiment uses the forward-KL score above as a sharper detector of overconfident missing-mass errors.
...
Table 4 confirms that the Q3 region carries real corrective signal.
...
Appendix B.5 provides concrete examples: a student that repeats a generic variable instead of substituting a concrete value (Ex. 1), an arithmetic computation error (Ex. 3), and a confident variable-level misstep in a derivation (Ex. 4)—all near-zero entropy, all strongly corrected by the teacher.

#### Q3 is not just “large divergence”.

Q3 tokens are by construction high-δ\delta, but the selector is structurally different from a pure divergence view.
Ranking by δt\delta_{t} alone at a budget matched to Q3-only underperforms the all-token baseline and only catches up when given 5×5\times more tokens (Appendix B.2, Table 8); it is the low-entropy conjunct that makes selection performant under tight budgets.
Conversely, high student entropy is not a proxy for high divergence—the two axes induce different selections and different performance curves (Table 3 vs. Table 8), and entropy-only rules are provably blind to Q3 (Proposition 2).
A related concern is that high-δ\delta tokens already yield the largest per-token gradients and thus should dominate the update; but in full-token training the aggregate update sums over all positions—mostly low-/moderate-δ\delta ones—and the operationally relevant question is which tokens are worth keeping under a budget, which Table 8 answers in favor of the low-entropy + high-divergence selector.

###
7.4 Type-Aware Selection (TIP)

The prediction of Remark 2 is that combining entropy with divergence should outperform entropy-only selection by recovering Q3 without sacrificing Q1/Q2.
Table 5 and Figure 1 present the comparison, and Appendix B.5 provides concrete token-level examples of the Q1/Q3 behaviors that the combined score is designed to retain.

Table 5: Main results: Baseline vs. Entropy-only vs. Soft-OR. Accuracy (%, mean@16 ±\pm std). Soft-OR uses st=h^t+δ^t−h^t⋅δ^ts_{t}=\hat{h}_{t}+\hat{\delta}_{t}-\hat{h}_{t}\cdot\hat{\delta}_{t} (Eq. 5) with Top-K selection. Bold marks the best per benchmark.
...
Table 6: Top 50% vs. bottom 50% by Soft-OR score. Accuracy (%, mean@16 ±\pm std). “Top” trains on the highest-scoring half by sts_{t}; “Bot.” trains on the lowest-scoring half. The bottom tokens carry substantially less signal.
...
Teacher entropy is nearly constant across positions and therefore provides little discriminative signal for token selection; details are deferred to Appendix B.1. The useful axes are the student’s state (hth_{t}) and the student–teacher gap (δt\delta_{t}), not the teacher’s uncertainty.

###
7.5 Beyond Mathematical Reasoning: Agentic Planning

The preceding experiments focus on mathematical reasoning.
To test whether TIP generalizes beyond this domain, we apply it to the DeepPlanning benchmark (Zhang et al., 2026) (Section 7.1).

#### Setup.

Using the Qwen3 Agentic pair described in Section 7.1, we train on 80% of the DeepPlanning Travel Planning tasks and evaluate on the remaining 20%.
...
Table 7: Agentic planning on DeepPlanning (Qwen3-1.7B student, thinking-enabled, Avg@16 %).
Left: reference and entropy-only methods. Right: Q3-only and Soft-OR.
Q3-only 20% surpasses full-token OPD; Soft-OR matches or exceeds entropy-only.
...
|
OPD, all tokens (100%)
11.7±0.0711.7{\pm}0.07
12.8±0.0712.8{\pm}0.07

|
+ Entropy-only 50%
12.1±0.0612.1{\pm}0.06
13.1±0.0713.1{\pm}0.07

|
+ Entropy-only 20%
11.6±0.0711.6{\pm}0.07
12.7±0.0612.7{\pm}0.06
...
|
OPD + Q3-only 20%
12.6±0.07\mathbf{12.6}{\pm}0.07
13.6±0.07\mathbf{13.6}{\pm}0.07

|
OPD + Soft-OR 50%
12.0±0.0612.0{\pm}0.06
13.1±0.0813.1{\pm}0.08

|
OPD + Soft-OR 20%
12.1±0.0612.1{\pm}0.06
12.6±0.0712.6{\pm}0.07

Table 7 confirms that TIP generalizes to a fundamentally different domain, with entropy-based selection at 50% retention matching or exceeding full-token OPD (12.1 vs. 11.7 for 14B; 13.1 vs. 12.8 for 32B).
...
Appendix B.3 provides Best@16 results, confirming the same pattern.
...
(1) Q3 detection requires teacher output distributions, though δt\delta_{t} is already part of the standard OPD loss.
...
L​(θ′)≤L​(θ)+⟨∇L​(θ),θ′−θ⟩+β2​‖θ′−θ‖2L(\theta^{\prime})\leq L(\theta)+\langle\nabla L(\theta),\theta^{\prime}-\theta\rangle+\frac{\beta}{2}\|\theta^{\prime}-\theta\|^{2}.

######
Assumption 2 (Token-separable approximation).

For tractability, we neglect off-diagonal gradient interactions across token positions. Concretely, for t≠st\neq s we treat the centered cross-token covariance

|

𝔼​[(gt−μ¯t)​(gs−μ¯s)⊤]\mathbb{E}[(g_{t}-\bar{\mu}_{t})(g_{s}-\bar{\mu}_{s})^{\top}]

as lower-order, so that the quadratic term admits a token-separable approximation.

###### Derivation.

Expand L​(θ−η​g^)L(\theta-\eta\hat{g}) via smoothness where g^=∑twt​gt\hat{g}=\sum_{t}w_{t}g_{t}:

|

L​(θ−η​g^)≤L​(θ)−η​⟨∇L,g^⟩+η2​β2​‖g^‖2.L(\theta-\eta\hat{g})\leq L(\theta)-\eta\langle\nabla L,\hat{g}\rangle+\frac{\eta^{2}\beta}{2}\|\hat{g}\|^{2}.

(8)

Using linearity of expectation and g^=∑twt​gt\hat{g}=\sum_{t}w_{t}g_{t},

|

𝔼​[−η​⟨∇L,g^⟩]=−η​∑twt​𝔼​[⟨∇L,gt⟩]=−η​∑twt​⟨∇L,μ¯t⟩=−η​∑twt​ϕ¯t.\mathbb{E}[-\eta\langle\nabla L,\hat{g}\rangle]=-\eta\sum_{t}w_{t}\,\mathbb{E}[\langle\nabla L,g_{t}\rangle]=-\eta\sum_{t}w_{t}\,\langle\nabla L,\bar{\mu}_{t}\rangle=-\eta\sum_{t}w_{t}\,\bar{\phi}_{t}.
...
𝔼​[‖g^‖2]=𝔼​[‖∑twt​gt‖2]=∑twt2​𝔼​[‖gt‖2]+∑t≠swt​ws​𝔼​[⟨gt,gs⟩].\mathbb{E}[\|\hat{g}\|^{2}]=\mathbb{E}\Bigl[\Bigl\|\sum_{t}w_{t}g_{t}\Bigr\|^{2}\Bigr]=\sum_{t}w_{t}^{2}\mathbb{E}[\|g_{t}\|^{2}]+\sum_{t\neq s}w_{t}w_{s}\,\mathbb{E}[\langle g_{t},g_{s}\rangle].

(10)

Write gt=μ¯t+(gt−μ¯t)g_{t}=\bar{\mu}_{t}+(g_{t}-\bar{\mu}_{t}). Under Assumption 2, we drop the off-diagonal covariance contribution and treat the remaining mean interaction terms as lower-order; absorbing these into the ≲\lesssim notation gives the token-separable approximation

|

𝔼​[‖g^‖2]≲∑twt2​M¯t.\mathbb{E}[\|\hat{g}\|^{2}]\lesssim\sum_{t}w_{t}^{2}\bar{M}_{t}.
...
𝔼​[L​(θ−η​g^)]−L​(θ)≲∑t(−η​wt​ϕ¯t+η2​β2​wt2​M¯t).\mathbb{E}[L(\theta-\eta\hat{g})]-L(\theta)\lesssim\sum_{t}\Bigl(-\eta w_{t}\bar{\phi}_{t}+\frac{\eta^{2}\beta}{2}w_{t}^{2}\bar{M}_{t}\Bigr).

(12)

The right-hand side is separable in tt, so minimizing each term gives

|

∂∂wt​(−η​wt​ϕ¯t+η2​β2​wt2​M¯t)=−η​ϕ¯t+η2​β​wt​M¯t=0,\frac{\partial}{\partial w_{t}}\Bigl(-\eta w_{t}\bar{\phi}_{t}+\frac{\eta^{2}\beta}{2}w_{t}^{2}\bar{M}_{t}\Bigr)=-\eta\bar{\phi}_{t}+\eta^{2}\beta w_{t}\bar{M}_{t}=0,

(13)

which yields wt∗=ϕ¯t/(η​β​M¯t)w_{t}^{*}=\bar{\phi}_{t}/(\eta\beta\bar{M}_{t}).
∎

###
A.2 Entropy-Weighted Sampling: Coverage and Variance

Sampling tokens with pt∝htp_{t}\propto h_{t} and using the importance-weighted estimator g^IS=1m​∑t∈Sgt/pt\hat{g}_{\mathrm{IS}}=\frac{1}{m}\sum_{t\in S}g_{t}/p_{t} is unbiased whenever pt>0p_{t}>0, with variance

|

Var​(g^IS)=1m2​∑t=1m1−ptpt​𝔼​[‖gt‖2].\mathrm{Var}(\hat{g}_{\mathrm{IS}})=\frac{1}{m^{2}}\sum_{t=1}^{m}\frac{1-p_{t}}{p_{t}}\,\mathbb{E}[\|g_{t}\|^{2}].

(14)

This makes the tradeoff transparent: entropy sampling preserves nonzero Q3 coverage, but the variance cost grows as 1/pt1/p_{t} for low-entropy tokens.

###
A.3 Why Adding Divergence Improves the Ranking

Under entropy-only scoring w^0=ht\hat{w}_{0}=h_{t}, Q3 and Q4 are both pushed to the low-score end.
The Soft-OR score st=h^t+δ^t−h^t⋅δ^ts_{t}=\hat{h}_{t}+\hat{\delta}_{t}-\hat{h}_{t}\cdot\hat{\delta}_{t} separates them: Q3 receives positive score from divergence (st≈δ^ts_{t}\approx\hat{\delta}_{t}) while Q4 remains near zero (both axes small).
Because the product term h^t⋅δ^t\hat{h}_{t}\cdot\hat{\delta}_{t} prevents double-counting, the high-entropy ranking is preserved and the score better tracks the oracle ordering without any tuning parameter.
...
Teacher entropy is near-zero everywhere (mean 0.0310.031, std 0.0550.055 for Qwen3; mean 0.0670.067, std 0.1640.164 for Llama; median token probability ≥0.79{\geq}0.79), so any scheme that conditions on teacher entropy—whether for token selection, loss weighting, or sampling—receives an almost constant input and therefore adds little discriminative information. In practice, the useful axes are the student’s state (hth_{t}) and the student–teacher gap (δt\delta_{t}), not the teacher’s uncertainty.

###
B.2 Isolating the Entropy Axis: Comparison with Divergence-Only Selection

Section 7.3 shows that low-entropy, high-divergence (Q3) tokens carry dense corrective signal.
A natural follow-up question is whether the low-entropy conjunct matters at all, or whether ranking by divergence δt\delta_{t} alone—ignoring student entropy—would suffice.
This is also the most direct comparison to the LATF module of AdaKD [Xie et al., 2026], which selects the top-rr% of tokens by teacher–student Hellinger distance: our Div-only top-kk selector is the same idea instantiated with reverse KL.
We deliberately give Div-only a much larger budget (top 50%) than Q3-only (top 10%): if the entropy axis were redundant, Div-only with 5×5\times more tokens should clearly dominate.

#### External reference point.

The published AdaKD ablation provides an independent data point for divergence-only selection.
On Qwen2-7B→\to1.5B (instruction-following, RKD baseline), their Table 3a reports that adding LATF alone changes ROUGE-L average from 37.0337.03 to 37.0737.07 (+0.04+0.04); the full +1.98 improvement of AdaKD comes from their orthogonal temperature-scaling module (IDTS), not from the token selector.
This is consistent with our Proposition 2 and motivates the present ablation in our setting.

#### Setup.

On Qwen3-8B (GRPO) →\to 4B with the standard mathematical-reasoning protocol (Section 7.1), we compare:
...
Div-only 50% (LATF-style hard top-kk): top 50% of tokens by δt\delta_{t} alone—no entropy term.

-
•

Div-only 10%: top 10% of tokens by δt\delta_{t} alone—budget-matched to Q3-only.

-
•

Q3-only 10%: top 10% of tokens by δtfwd⋅(1−h^t)\delta_{t}^{\mathrm{fwd}}\cdot(1-\hat{h}_{t}) (Section 7.3)—explicitly requires both low entropy and high divergence.

This design isolates two questions: (i) at 5×5\times fewer tokens, does adding the low-entropy filter beat divergence alone (Div-only 50% vs Q3-only 10%)? and (ii) at equal budget, does the low-entropy filter beat pure divergence ranking (Div-only 10% vs Q3-only 10%)?

Table 8: Isolating the entropy axis: Div-only vs Q3-only on Qwen3-8B (GRPO) →\to 4B (mean@16 %, ±\pm std). Div-only is a hard top-kk instantiation of LATF-style divergence selection [Xie et al., 2026]. We compare Q3-only at 10% against Div-only at both 5×5\times the budget (50%) and matched budget (10%). Bold marks the best per benchmark.
...
Table 8 resolves both questions and sharpens what the entropy axis contributes.

(i) Equal budget (Div-only 10% vs Q3-only 10%).
At the same 10% retention, Q3-only beats Div-only on every benchmark: MATH-500 76.176.1 vs 74.374.3 (+1.8+1.8), AIME’24 21.521.5 vs 19.219.2 (+2.3+2.3), AIME’25 17.117.1 vs 15.315.3 (+1.8+1.8).
Div-only at 10% drops well below the all-token baseline (−2.4,−2.7,−4.1-2.4,-2.7,-4.1), while Q3-only stays within 0.60.6 of the baseline on MATH/AIME’24.
With the divergence ranking and budget held fixed, simply requiring tokens to also be low-entropy—i.e., re-weighting by (1−h^t)(1-\hat{h}_{t})—is what closes the gap to the baseline.

(ii) 5×5\times budget gap (Div-only 50% vs Q3-only 10%).
Once Div-only is given 5×5\times the tokens (50%), it largely catches up: MATH-500 76.376.3 vs Q3-only’s 76.176.1 (+0.2+0.2), AIME’24 a tie at 21.521.5, AIME’25 21.121.1 vs 17.117.1 (+4.0+4.0 in Div-only’s favor).
This is consistent with the fact that, as rr grows, the high-divergence tail necessarily includes most Q3 tokens as a side effect.
The headline is per-token efficiency: Q3-only matches Div-only on two of three benchmarks while using 5×5\times fewer tokens.
...
Q3 is, by construction, a high-δ\delta region; under a sharp teacher distribution it can also overlap with high-entropy student tokens.
The point is structural rather than set-theoretic.
Prior work on token-level importance has mostly interpreted “which tokens matter” through the lens of student uncertainty—high entropy is taken as the proxy for an informative position—and entropy-based rules are provably blind to confident-but-wrong tokens (Proposition 2).
...
Table 8 makes this concrete from two directions. Large-divergence ranking on its own is not sufficient: Div-only at a budget matched to Q3-only is consistently weaker than the joint score, and only recovers when given a 5×5\times larger budget—i.e., when it also sweeps up much of the low-entropy tail.
Conversely, large student entropy is not the same selector as large divergence: high-hh tokens need not be high-δ\delta, and—as the entropy sweeps in Table 3 show—the two selectors trace different performance curves across retention ratios.
...
The Div-only 50% column is the within-our-setup analogue of AdaKD’s published LATF-only ablation [Xie et al., 2026]: a hard top-rr% divergence selector with no entropy interaction.
Their Table 3a reports +0.04+0.04 ROUGE-L for LATF alone on Qwen2-1.5B; we observe a similarly small effect on average (−0.4-0.4 on MATH-500, −0.4-0.4 on AIME’24, +1.7+1.7 on AIME’25 relative to the all-token baseline).
The takeaway is the same in both settings: divergence ranking on its own is roughly baseline-equivalent at large budgets and clearly worse at small budgets, while a low-entropy + high-divergence selector is competitive at both—consistent with Proposition 2 and Remark 2.

###
B.3 Agentic Planning: Held-Out Queries

Figure 4: Token selection for agentic OPD on 20% held-out travel-planning queries.
Top row: Avg@16; Bottom row: Best@16 (Pass@16).
Within each row the left panel uses the 14B teacher and the right panel uses the 32B teacher.
Q3-only 20% matches or exceeds the full-token baseline in every setting, consistent with Table 7.
...
Figure 4 complements Table 7 with a finer-grained view.
...
For entropy sampling experiments, tokens are sampled with probability pt∝htp_{t}\propto h_{t}, with the retention ratio determining how many tokens are kept.
For Top-K experiments, the top-ρ\rho fraction of tokens by score is selected deterministically.
...
Student entropy: 0.020.02 (extremely confident). Forward KL: 5.275.27. Overconfidence score: 5.245.24.

Problem

Let 𝒫\mathcal{P} be a set of monic polynomials with integer coefficients of the least degree, with root k⋅cos⁡(4​π7)k\cdot\cos\!\left(\frac{4\pi}{7}\right), as kk spans over the positive integers. Let P​(x)∈𝒫P(x)\in\mathcal{P} be the polynomial so that |P​(1)||P(1)| is minimized. Find the remainder when P​(2017)P(2017) is divided by 10001000.

Student response (excerpt)

…The polynomial with roots of the form k⋅cos⁡(4​π7)\hbox{\pagecolor{red!20}{k}}\cdot\cos\!\left(\frac{4\pi}{7}\right) can be constructed by considering the minimal polynomial of cos⁡(4​π7)\cos\!\left(\frac{4\pi}{7}\right)…

Token distributions

Student top-5: k (99.8%), x (0.1%), e (0.0%), a (0.0%), z (0.0%)

Teacher top-5: 2 (49.9%), k (30.3%), x (8.7%), e (6.8%), z (2.8%)
...
An entropy-only rule would assign near-zero weight to this position because ht=0.02h_{t}=0.02, yet it carries one of the densest corrective signals in the batch (overconfidence score =5.24=5.24).

#### Example 2: Reasoning fork—restating vs. advancing (Q1: high entropy, high divergence).

Student entropy: 1.821.82 (uncertain). Forward KL: 5.315.31.

Problem

An arithmetic sequence of positive integers has n≥3n\geq 3 terms, initial term aa, and common difference d>1d>1. Carl wrote down all the terms correctly except for one term, which was off by 11. The sum of the terms he wrote was 222222. What is a+d+na+d+n?

Student response (excerpt)

…Given ai=a+ka_{i}=a+k where k=1k=1 if the term a+k​da+kd (for some integer kk) was off and 11 if the term a+(k+1)​da+(k+1)d was off.…

Token distributions

Student top-5: off (54.4%), missing (15.6%), the (9.5%), not (4.5%), replaced (3.1%)

Teacher top-5: written (40.3%), increased (35.6%), off (7.9%), decreased (2.9%), incremented (2.0%)
...
This is a classic Q1 token: the student is uncertain (entropy =1.82=1.82) and the teacher strongly disagrees—entropy-based selection would catch this token.

#### Example 3: Arithmetic computation error (Q3: low entropy, high divergence).

Student entropy: 0.400.40. Forward KL: 3.543.54. Overconfidence score: 3.273.27.

Problem

Find the sum of the real roots of f​(x)=x4+9​x3+18​x2+18​x+4f(x)=x^{4}+9x^{3}+18x^{2}+18x+4.

Student response (excerpt)

…f​(2)=24+9⋅23+18⋅22+18⋅2+4=16+72+72+36+4=19​0≠0f(2)=2^{4}+9\cdot 2^{3}+18\cdot 2^{2}+18\cdot 2+4=16+72+72+36+4=19\hbox{\pagecolor{red!20}{0}}\neq 0…

Token distributions

Student top-5: 0 (91.2%), 8 (4.0%), 2 (3.5%), 4 (0.7%), 6 (0.5%)

Teacher top-5: 6 (60.0%), 2 (25.0%), 8 (6.3%), 9 (3.8%), 0 (3.0%)

Analysis.
The student confidently writes f​(2)=190f(2)=190 with 91.2% on digit 0, committing an arithmetic error in summing 16+72+72+36+416+72+72+36+4.
The teacher distributes probability across 6 (60.0%) and 2 (25.0%), clearly disagreeing with 0.
Like Example 1, this is a Q3 token: the student is fairly confident (ht=0.40h_{t}=0.40), yet the teacher strongly disagrees—entropy-based selection would under-weight this position.

#### Example 4: Confident on the wrong variable (Q3: low entropy, high divergence).

Student entropy: 0.120.12 (very confident). Forward KL: 5.585.58. Overconfidence score: 5.445.44.

Problem

If the three interior angles A,B,CA,B,C of triangle △​A​B​C\triangle ABC have cotangents cot⁡A,cot⁡B,cot⁡C\cot A,\cot B,\cot C that form an arithmetic sequence, and the maximum value of angle BB can be expressed as m​πn\frac{m\pi}{n}, find the value of m+nm+n.

Student response (excerpt)

…Next, consider the cotangent of each angle cot⁡B=sin⁡Bcos⁡B\cot B=\frac{\sin\hbox{\pagecolor{red!20}{B}}}{\cos B}, based on the arithmetic sequence property that cot⁡B−cot⁡A\cot B-\cot A and cot⁡B−cot⁡C\cot B-\cot C are equal…

Token distributions

Student top-5: B (98.2%), (B (1.0%), \’ (0.2%), ˆ (0.2%), C (0.1%)

Teacher top-5: A (52.4%), (A (11.7%), C (11.7%), B (6.3%), ( (6.3%)

Analysis.
The student assigns 98.2% to B, while the teacher prefers A (52.4%).
The student writes cot⁡B=sin⁡Bcos⁡B\cot B=\frac{\sin B}{\cos B}, which is mathematically incorrect (cot⁡B=cos⁡Bsin⁡B\cot B=\frac{\cos B}{\sin B}); the teacher’s preferred continuation reflects a different and more correct reasoning path.
Another clear Q3 token: ht=0.12h_{t}=0.12 would be invisible to entropy-only selection.
...
Student entropy: 1.381.38. Forward KL: 4.274.27.
...
…Assume the common difference between two adjacent terms is equal, i.e., b−a=\b-a=\backslashDelta (where Δ\Delta is a constant)…

Token distributions

Student top-5: text (53.5%), Delta (25.3%), frac (13.5%), lambda (1.4%), pm (1.0%)

Teacher top-5: cot (91.7%), frac (4.6%), text (1.1%), delta (1.0%), Delta (0.8%)
